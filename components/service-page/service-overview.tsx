...
export default function ServiceOverview({ service }: ServiceOverviewProps) {
  ...
  return (
    <section className="bg-[#1D1D1D] border-t border-[#4A4A4A]">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 pt-12 md:pt-16 lg:pt-20 pb-12 md:pb-16 lg:pb-20">
        <FadeIn className="mb-12 md:mb-16 lg:mb-20" duration={0.6}>
          ...
          <h2 className="text-white font-noto-serif text-center mt-6" style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: 600, lineHeight: '1.5', letterSpacing: '0px', fontFamily: 'Noto Serif, serif', color: '#FFFFFF' }}>
            Service Overview
          </h2>
        </FadeIn>
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12">
          ...
          {service.overview?.map((text, i) => (
              <p key={i} className="text-white/90 font-dm-sans" style={{ fontSize: 'clamp(1rem, 1.25vw, 1.125rem)', fontWeight: 400, lineHeight: '1.625', letterSpacing: '0px', fontFamily: 'DM Sans, sans-serif', color: 'rgba(255, 255, 255, 0.9)' }}>
              {text}
            </p>
          ))}
          </FadeIn>
          ...
          <FadeIn direction="left">
            <div className="rounded-[24px] border border-[#4A4A4A] bg-white shadow-[0_18px_45px_rgba(0,0,0,0.1)] p-6 sm:p-8">
              <h3 className="font-semibold text-[#1D1D1D] mb-5 font-noto-serif" style={{ fontSize: 'clamp(1.25rem, 1.5vw, 1.5rem)', fontWeight: 500, lineHeight: '1.5', letterSpacing: '0px', fontFamily: 'Noto Serif, serif', color: '#1D1D1D' }}>
            Key Benefits
          </h3>
            ...
            {service.benefits?.map((item, i) => (
                  ...
                  >
                    <CheckCircle className="text-[#F04E22] mt-1 shrink-0" size={18} />
                    <span>{item}</span>
                  </li>
            ))}
          </ul>

              <h4 className="font-semibold text-[#1D1D1D] mt-6 mb-3 font-noto-serif" style={{ fontSize: 'clamp(1rem, 1.5vw, 1.5rem)', fontWeight: 400, lineHeight: '1.2', letterSpacing: '0px', fontFamily: 'Noto Serif, serif', color: '#1D1D1D' }}>
            Ideal For
          </h4>
              <ul className="text-[#7F8C8D] space-y-2 mb-6 font-dm-sans" style={{ fontSize: 'clamp(0.875rem, 1.25vw, 1rem)', fontWeight: 400, lineHeight: '1.2', letterSpacing: '0px', fontFamily: 'DM Sans, sans-serif', color: '#7F8C8D' }}>
            {service.audience?.map((item, i) => (
              ...
              {relatedPosts.length > 0 && (
                <div className="pt-6 border-t border-[#4A4A4A]">
                  <h4 className="font-semibold text-[#1D1D1D] mb-3 font-noto-serif" style={{ fontSize: 'clamp(1rem, 1.5vw, 1.5rem)', fontWeight: 400, lineHeight: '1.2', letterSpacing: '0px', fontFamily: 'Noto Serif, serif', color: '#1D1D1D' }}>
                    Related Resources
                  </h4>
                  <ul className="space-y-2">
                    {relatedPosts.map((post, i) => (
                      <li key={i}>
                        <Link
                          href={`/blog/${post.slug}`}
                          className="text-sm text-[#214B57] hover:opacity-80 hover:underline transition font-dm-sans"
