            ...
            return (
              <FadeIn key={i} direction="up" delay={0.3 + (i * 0.15)} duration={0.6}>
                <div className="relative bg-white border border-[#4A4A4A] p-8 rounded-[28px] text-center hover:shadow-[0_25px_70px_rgba(0,0,0,0.1)] transition-all group">
                  ...
                  <h4 className="font-semibold text-[#1D1D1D] mb-3 font-noto-serif" style={{ fontSize: 'clamp(1.125rem, 1.5vw, 1.25rem)', fontWeight: 500, lineHeight: '1.5', letterSpacing: '0px', fontFamily: 'Noto Serif, serif', color: '#1D1D1D' }}>
                {value.title}
              </h4>
                  <p className="text-[#7F8C8D] text-sm sm:text-base leading-relaxed font-dm-sans" style={{ fontSize: 'clamp(0.875rem, 1.25vw, 1rem)', fontWeight: 400, lineHeight: '1.625', letterSpacing: '0px', fontFamily: 'DM Sans, sans-serif', color: '#7F8C8D' }}>
                {value.desc}
              </p>
            </div>
              </FadeIn>
